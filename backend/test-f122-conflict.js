import { detectDependencyConflicts } from "./dist/utils/customCodeValidator.js";
...
function runTest() {
    ...
    const mismatchList = [{
        ...
        dependencies: [
            { type: "javascript", url: "https://example.com/style.css" },
            { type: "css", url: "https://example.com/script.js" }
        ]
    }];
    ...
    const versionConflictList = [
        {
            ...
            conditions: [],
            environments: ["production"],
            dependencies: [{ type: "javascript", url: "https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.min.js" }]
        },
        {
            id: "snip-jquery2",
            ...
            conditions: [],
            environments: ["production"],
            dependencies: [{ type: "javascript", url: "https://cdnjs.cloudflare.com/ajax/libs/jquery/2.2.4/jquery.min.js" }]
        }
    ];
    ...
    const hasError = verConflicts.some(c => c.severity === "error" && c.type === "version_conflict");
    const hasWarning = verConflicts.some(c => c.severity === "warning" && c.type === "duplicate_library");
    if (!hasError || !hasWarning) {
        throw new Error("FAILED: Incompatible jQuery versions in active environment must trigger error & warning: hasError=" + hasError + " hasWarning=" + hasWarning);
    }
    ...
    if (condConflicts.length !== 0) {
        console.log("Actual conflicts found in test 4:", JSON.stringify(condConflicts, null, 2));
    ...
    const missingDependencyList = [{
        ...
        conditions: [],
        environments: ["production"],
        dependencies: [{ type: "javascript", url: "https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.13/js/select2.min.js" }]
    }];
    ...
    const orderConflictList = [{
        ...
        environments: ["production"],
        dependencies: [
            { type: "javascript", url: "https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.13/js/select2.min.js" },
            { type: "javascript", url: "https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.min.js" }
        ]
    }];
    ...
    const circularList = [{
        ...
        environments: ["production"],
        dependencies: [
            { type: "javascript", url: "https://example.com/libA.js", requires: "libB.js" },
            { type: "javascript", url: "https://example.com/libB.js", requires: "libA.js" }
        ]
    }];
    const circConflicts = detectDependencyConflicts(circularList);
    console.log("Circular dependency conflicts found:", JSON.stringify(circConflicts, null, 2));
    const hasCircular = circConflicts.some(c => c.severity === "error" && c.type === "circular_dependency");
    if (!hasCircular) {
