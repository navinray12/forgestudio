...
export async function downloadLatestStaticExportHandler(req: Request, res: Response, next: NextFunction) {
  try {
    const websiteId = String(req.params.id);
    const userId = res.locals.user?.id;
    const { getWebsiteById } = await import("../websites/website.service.js");
    const website = await getWebsiteById(websiteId, userId);
    const snapshot = typeof website.editorData === "string" ? JSON.parse(website.editorData) : (website.editorData || {});
    const { compileCanonicalToStaticBundle } = await import("./destinations/static-compiler.js");
