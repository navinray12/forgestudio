/**
 * @file Revisions: business operations and coordination with persistence or external services. File responsibility: revision service.
 ...
 * @param websiteId Identifier of the website whose data is being read or changed.
 * @param userId User identifier used to scope this operation; authorization is checked by the relevant caller or service.
 * @param payload Payload supplied to this operation (type: CreateRevisionInput).
 */
...
export async function restoreRevision(websiteId: string, revisionId: string, userId: string) {
  ...
  const updatedWebsite = await saveDraftPreservingPublishedSnapshot(websiteId, updatedEditorData);
