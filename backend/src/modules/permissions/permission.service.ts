/**
 * @file Permissions: business operations and coordination with persistence or external services. File responsibility: permission service.
...
export const DEFAULT_CAPABILITIES: Record<string, string[]> = {
  ...
  DESIGNER: [
    ...
    "EDIT",
    "CREATE",
    // Design access does not grant publication; an explicit stored ALLOW is required.
    "COMMENT",
    "EDIT_CONTENT",
    "EDIT_DESIGN",
  ],
 ...
 * @param resourceId Resource Id supplied to this operation (type: string).
 * @param capability Capability supplied to this operation (type: string).
...
export async function canUserAccessResource(
  ...
  if (!userId || !websiteId || !resourceId || !capability) {
    throw new AppError("Authorization requires a user, website, resource and capability.", 400, "MISSING_AUTHORIZATION_CONTEXT");
  }
  ...
  try {
    {
      ...
      if (resourceId !== "*") {
        const specificOverride = await client.granularPermission.findUnique({
          where: {
      ...
      const globalOverride = await client.granularPermission.findUnique({
        where: {
          websiteId_userId_resourceId_capability: {
            websiteId,
            userId,
    ...
    throw new AppError("Permission policies could not be evaluated. Try again later.", 503, "AUTHORIZATION_UNAVAILABLE");
  }
 ...
 * @param capability Capability supplied to this operation (type: string).
 * @param resourceId Resource Id supplied to this operation (type: string). Defaults to "*".
 */
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (typeof websiteId !== "string" || !websiteId.trim()) {
        throw new AppError("Website context is required for authorization.", 400, "MISSING_AUTHORIZATION_CONTEXT");
      }
...
/**
 * Set Granular Permission.
 * @param websiteId Identifier of the website whose data is being read or changed.
 * @param requesterUserId Requester User Id supplied to this operation (type: string).
 * @param targetUserId Target User Id supplied to this operation (type: string).
 * @param resourceId Resource Id supplied to this operation (type: string).
 * @param capability Capability supplied to this operation (type: string).
 * @param effect Effect supplied to this operation (type: string).
 */
