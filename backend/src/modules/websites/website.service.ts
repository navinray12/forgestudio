...
export async function getManagedWebsiteDetails(websiteId: string, userId: string) {
  ...
  try {
    const { getMailerConfig } = await import("../publishing/siteMailer.service.js");
  ...
  try {
    const { getDeliveryLogs } = await import("../publishing/siteMailer.service.js");
  ...
  try {
    const { getPerformanceSummary } = await import("../publishing/sitePerformance.service.js");
    performanceStats = await getPerformanceSummary(websiteId, userId);
  } catch {}

  try {
    const { getOptimizationStats } = await import("../publishing/imageOptimization.service.js");
    optimizationStats = await getOptimizationStats(websiteId, userId);
  } catch {}

  let staging: any = null;
  try {
    const { getStagingEnvironment } = await import("../publishing/staging.service.js");
...
export async function updateWebsiteEditorData(
  ...
  const safePopups = isAdmin
    ? (incomingEditorData.popups ?? currentEditorData.popups ?? [])
    : (currentEditorData.popups || []).map((p: any) => {
  ...
  try {
    ...
    return await saveDraftPreservingPublishedSnapshot(websiteId, finalEditorData, client);
  } catch (error) {
