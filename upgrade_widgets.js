...
// 1. We replace the HTML settings logic
// I need to use regex to find the entire HTML and Shortcode blocks to replace them robustly.
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          ...
                          {(() => {
                            ...
                            if (isValid) return <span className="text-[9px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded">✓ Valid</span>;
                        ...
                        <div className="rounded-lg border border-slate-700 bg-[#0d1117] overflow-hidden shadow-inner flex flex-col">
                          <div className="flex items-center justify-between px-3 py-2 bg-[#161b22] border-b border-slate-800">
                             ...
                             <svg className="w-3.5 h-3.5 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg>
                          </div>
                          <textarea
                            rows={12}
                            value={tempHtml}
                        ...
                        <div className="flex items-center flex-wrap gap-1.5 pt-1">
                           ...
                           <button onClick={() => setTempHtml('<div class="p-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl font-bold shadow-lg text-center">💎 Premium Promo Card</div>')} className="px-2 py-1 bg-slate-50 hover:bg-blue-50 text-slate-600 hover:text-blue-600 rounded text-[9px] font-bold border border-slate-200 transition">✨ Promo Card</button>
                        </div>
                      </div>

                      <div className="mt-1">
                        <label className="flex items-center gap-2 cursor-pointer bg-red-50/30 hover:bg-red-50 p-2.5 rounded-lg border border-red-100 transition" title="Bypasses sanitization in preview mode">
                          <input
                             type="checkbox"
                        ...
                        <div className="flex flex-col gap-1.5 pt-2">
                           ...
                           <div className="grid grid-cols-2 gap-2">
                             <button onClick={() => setTempShortcode('[contact-form]')} className="flex items-center gap-1.5 px-2 py-1.5 bg-slate-50 hover:bg-blue-50 text-slate-700 hover:text-blue-700 rounded-md text-[10px] font-bold border border-slate-200 transition text-left">
                                <span className="text-blue-500">✉️</span> Contact Form
                             </button>
                             <button onClick={() => setTempShortcode('[blog-feed]')} className="flex items-center gap-1.5 px-2 py-1.5 bg-slate-50 hover:bg-blue-50 text-slate-700 hover:text-blue-700 rounded-md text-[10px] font-bold border border-slate-200 transition text-left">
                                <span className="text-orange-500">📰</span> Latest News
                             </button>
                             <button onClick={() => setTempShortcode('[pricing-table]')} className="flex items-center gap-1.5 px-2 py-1.5 bg-slate-50 hover:bg-blue-50 text-slate-700 hover:text-blue-700 rounded-md text-[10px] font-bold border border-slate-200 transition text-left">
                                <span className="text-emerald-500">💳</span> Pricing Tiers
                             </button>
...
// Replace using string boundary markers since we know what it starts/ends with exactly.
const startMarker = '{/* HTML Type Settings (F-110) */}';
const endMarker = '{/* F-108 & F-109 */}'; // We know custom attributes comes after shortcode
