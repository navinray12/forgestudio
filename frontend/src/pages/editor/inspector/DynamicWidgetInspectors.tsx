...
import { resolveControl } from "../../../editor/core/registry/controlRuntime";
import { WpMediaPickerModal, wordpressApi } from "../../../integrations/wordpress";


/**
 * resolveInspectorControlDefinition
 * Phase 5 Control Registry Integration Point:
 * Queries ControlRuntime (ControlRegistry) for control definitions,
 * ensuring control lookup succeeds with seamless fallback to existing inspector controls.
 */
export function resolveInspectorControlDefinition(controlType: string) {
  const definition = resolveControl(controlType);
  if (definition) {
    return definition;
  }
  return undefined;
...
export function ImagePickerControl({
  ...
  label?: string;
  websiteId?: string;
  onSelectWpMedia?: (item: any) => void;
}) {
  const [showPresets, setShowPresets] = React.useState(false);
  const [showWpModal, setShowWpModal] = React.useState(false);
    ...
    <div className="space-y-1.5">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          placeholder="https://images.unsplash.com/... or select image"
        ...
        <button
          ...
          className="shrink-0 flex items-center gap-1 rounded-lg bg-slate-800 hover:bg-slate-900 text-white px-2.5 py-1.5 text-xs font-bold shadow-xs transition active:scale-95 cursor-pointer"
      ...
      {websiteId && (
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 rounded-lg border border-blue-200 bg-blue-50 hover:bg-blue-100 text-blue-700 py-1.5 text-xs font-bold transition shadow-2xs cursor-pointer"
        >
          <span>🌐 Choose WordPress Image</span>
        </button>
      )}
          ...
          {currentItems.map((item, idx) => {
            ...
            return (
              <div
                ...
                className={`rounded-xl border p-3 shadow-xs space-y-2.5 transition duration-150 ${
                  isBeingDragged
                    ? "opacity-40 border-dashed border-blue-400 bg-blue-50/50 scale-[0.99]"
                    : isDragOver
                ...
                <div className="flex items-center justify-between border-b border-slate-100 pb-1.5">
                  <div className="flex items-center gap-1.5 truncate max-w-[170px]">
                    <span
                      ...
                      className="cursor-grab active:cursor-grabbing text-slate-400 hover:text-slate-700 px-1 py-0.5 text-xs font-bold rounded hover:bg-slate-100 select-none"
                    >
                      ⋮⋮
                    </span>
...
export function ImageWidgetInspector({
  ...
  const handleWpMediaSelect = (mediaItem: any) => {
    updateProp("src", mediaItem.url);
    if (mediaItem.alt || mediaItem.title) {
      updateProp("alt", mediaItem.alt || mediaItem.title);
    }
    updateProp("mediaId", mediaItem.id);
    updateProp("mediaSource", "wordpress");
  };

  const handleRemoveImage = () => {
    updateProp("src", "");
    updateProp("mediaId", undefined);
    updateProp("mediaSource", undefined);
  };
  ...
  return (
    <div className="space-y-3 pt-2 border-t border-slate-100">
      ...
      <ImagePickerControl
        ...
        websiteId={websiteId}
        onSelectWpMedia={handleWpMediaSelect}
      />

      {el.mediaId && el.mediaSource === "wordpress" && (
        <div className="rounded-lg bg-blue-50/70 border border-blue-200/80 p-2 text-[11px] text-blue-800 flex items-center justify-between">
          <span className="font-semibold">🌐 WP Attachment #{el.mediaId}</span>
          <span className="text-[10px] bg-blue-200/60 text-blue-900 px-1.5 py-0.5 rounded-xs font-bold uppercase">Linked</span>
        </div>
      )}
...
function convertWpItemsToNavMenuItems(wpItems: any[]): NavMenuItem[] {
  const topLevel = wpItems.filter((item) => !item.parentId || item.parentId === 0);
  ...
  return topLevel
    ...
    .map((top) => {
      const subs = childrenMap.get(top.id) || [];
      const navItem: NavMenuItem = {
        id: String(top.id),
        label: top.title || "Item",
        url: top.url || "#",
        target: top.target === "_blank" ? "_blank" : "_self",
      };
      if (subs.length > 0) {
        navItem.dropdownEnabled = true;
        navItem.submenu = subs
          .sort((a, b) => (a.order || 0) - (b.order || 0))
          .map((s) => ({
            id: String(s.id),
            label: s.title || "Subitem",
            url: s.url || "#",
            target: s.target === "_blank" ? "_blank" : "_self",
          }));
      }
      return navItem;
    });
...
function convertNavMenuItemsToWpItems(items: NavMenuItem[]): any[] {
  const result: any[] = [];
  let orderCounter = 1;

  for (const item of items) {
    const parentId = parseInt(item.id, 10) || 0;
    result.push({
      id: parentId,
      title: item.label,
      url: item.url,
    ...
    if (item.submenu && item.submenu.length > 0) {
      for (const sub of item.submenu) {
        const subId = parseInt(sub.id, 10) || 0;
        result.push({
          id: subId,
          title: sub.label,
          url: sub.url,
...
export function NavMenuWidgetInspector({
  ...
  const menuSource = el.menuSource || "forgestudio";

  const [wpMenus, setWpMenus] = React.useState<Array<{ id: number; name: string; count?: number }>>([]);
  const [loadingMenus, setLoadingMenus] = React.useState<boolean>(false);
  const [loadingSelectedMenu, setLoadingSelectedMenu] = React.useState<boolean>(false);
  const [savingWpMenu, setSavingWpMenu] = React.useState<boolean>(false);
  const [wpStatusMsg, setWpStatusMsg] = React.useState<{ type: "success" | "error" | null; text: string }>({ type: null, text: "" });

  React.useEffect(() => {
    if (menuSource === "wordpress" && websiteId) {
      setLoadingMenus(true);
      setWpStatusMsg({ type: null, text: "" });
      wordpressApi.getMenus(websiteId)
        .then((res) => {
          setWpMenus(res.menus || []);
          setLoadingMenus(false);
        })
        .catch((err) => {
          console.warn("[NavMenuWidgetInspector] Failed to fetch WP menus:", err);
          setWpStatusMsg({ type: "error", text: err?.message || "Failed to fetch WordPress menus" });
          setLoadingMenus(false);
        });
    }
  }, [menuSource, websiteId]);

  const handleSelectWpMenu = async (menuIdStr: string) => {
    updateProp("wpMenuId", menuIdStr);
    if (!menuIdStr || !websiteId) return;

    setLoadingSelectedMenu(true);
    setWpStatusMsg({ type: null, text: "" });
    try {
      const res = await wordpressApi.getMenu(websiteId, menuIdStr);
      if (res.menu && res.menu.items) {
        const converted = convertWpItemsToNavMenuItems(res.menu.items);
        updateProp("navMenuItems", converted);
        setWpStatusMsg({ type: "success", text: `Loaded ${res.menu.items.length} items from "${res.menu.name}"` });
      }
    } catch (err: any) {
      console.error("[NavMenuWidgetInspector] Failed to load menu details:", err);
      setWpStatusMsg({ type: "error", text: err?.message || "Failed to load WordPress menu items" });
    } finally {
  ...
  const handleSaveToWordPress = async () => {
    if (!websiteId || !el.wpMenuId) return;
    setSavingWpMenu(true);
    setWpStatusMsg({ type: null, text: "" });
    try {
      const flatItems = convertNavMenuItemsToWpItems(items);
      const res = await wordpressApi.updateMenu(websiteId, el.wpMenuId, flatItems);
      setWpStatusMsg({ type: "success", text: res.message || "WordPress Menu saved successfully!" });
    } catch (err: any) {
      console.error("[NavMenuWidgetInspector] Save error:", err);
      setWpStatusMsg({ type: "error", text: err?.message || "Failed to save menu to WordPress" });
    } finally {
      setSavingWpMenu(false);
    }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-3 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            type="button"
            onClick={() => updateProp("menuSource", "forgestudio")}
          ...
          <button
            type="button"
            onClick={() => updateProp("menuSource", "wordpress")}
          ...
          <div className="space-y-2 pt-2 border-t border-slate-200">
            {!websiteId ? (
              <div className="text-[11px] text-amber-700 bg-amber-50 p-2.5 rounded-lg border border-amber-200">
                ⚠️ Connect a WordPress website to manage WordPress Navigation Menus.
              </div>
            ) : loadingMenus ? (
              <div className="text-[11px] text-slate-500 animate-pulse py-1">
                Loading WordPress menus...
              </div>
            ) : (
              <>
                <div>
                  ...
                  <select
                    value={el.wpMenuId || ""}
                    onChange={(e) => handleSelectWpMenu(e.target.value)}
                    disabled={loadingSelectedMenu}
                    className="w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs font-medium focus:ring-2 focus:ring-blue-500 outline-none"
                  >
                    <option value="">-- Choose WP Menu --</option>
                    {wpMenus.map((m) => (
                      <option key={m.id} value={String(m.id)}>
                        {m.name} {m.count !== undefined ? `(${m.count} items)` : ""}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                {el.wpMenuId && (
                  <button
                    ...
                    disabled={savingWpMenu}
                    className="w-full py-2 px-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 active:scale-98 text-white text-xs font-bold shadow-xs transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                  >
                    {savingWpMenu ? (
                      <>
                      ...
                      <>
                        <span>💾 Save Menu to WordPress</span>
                      </>
                    )}
                  </button>
                )}
                ...
                {wpStatusMsg.text && (
                  <div
                    className={`text-[11px] p-2 rounded-lg border font-medium ${
                      wpStatusMsg.type === "success"
                        ? "bg-emerald-50 text-emerald-800 border-emerald-200"
                        : "bg-rose-50 text-rose-800 border-rose-200"
                    }`}
                  >
                    {wpStatusMsg.type === "success" ? "✓ " : "⚠️ "}
                    {wpStatusMsg.text}
                  </div>
                )}
              </>
...
export function ButtonWidgetInspector({
  ...
  const currentButtonColor = el.buttonBg || el.styles?.backgroundColor || "#2563eb";
  const currentContainerBg = el.containerBg || "transparent";
  const currentTextColor = el.buttonColor || el.styles?.color || "#ffffff";
