        ...
        <div className="space-y-2.5">
          {plans.map((plan, planIdx) => {
            ...
            return (
              <div
                key={plan.id}
                className={`rounded-xl border transition-all duration-200 bg-white ${isHighlight ? "border-amber-300 ring-1 ring-amber-200" : "border-slate-200"
                  }`}
              >
                {/* Accordion Header */}
                <div
                        ...
                        {(plan.features || []).map((feat, featIdx) => (
                          <div key={feat.id} className="flex items-center gap-1.5 bg-slate-50 p-1.5 rounded-lg border border-slate-200">
                            ...
                            <button
                              ...
                              className={`h-5 w-5 rounded text-[10px] font-extrabold shrink-0 flex items-center justify-center cursor-pointer transition ${feat.included ? "bg-emerald-100 text-emerald-700 border border-emerald-300" : "bg-slate-200 text-slate-500 border border-slate-300"
                                }`}
                            >
                              {feat.included ? "✓" : "✕"}
                            </button>
