...
import { useEditorDocument, useEditorUIState, useEditorViewState, useEditorWorkspaceState, useEditorInteractionState, useEditorHistory } from "../../editor/core/state";
...
export default function WebsiteEditor(props?: WebsiteEditorProps) {
...
const [popups, setPopups] = useState<any[]>([]);
  ...
  // Phase 7 UI State Extraction Point (Selection & Hover Foundation)
  const _editorUISelection = useEditorUIState({
    selectedId,
    selectedIds,
    hoveredId,
    setSelectedId,
  ...
  // Phase 10 Canvas Interaction State Extraction Point (Transient Drag/Drop Foundation)
  const _editorInteractionState = useEditorInteractionState({
    draggingId,
    setDraggingId,
    dropTargetId,
    setDropTargetId,
  ...
  // Phase 11 Command & History Integration Point (Controlled History Adapter)
  const _editorHistoryState = useEditorHistory({
    history,
    historyIndex,
    setHistory,
    setHistoryIndex,
  ...
  // Phase 9 Workspace State Extraction Point (Sidebar & Layer Collapsing Foundation)
  const _editorWorkspaceState = useEditorWorkspaceState({
    leftSidebarTab,
    setLeftSidebarTab,
    activeSidebarTab,
    setActiveSidebarTab,
...
const [activePageId, setActivePageId] = useState<string>("home");
  ...
  // Phase 6 Document State Extraction (Proof of Concept)
  const { activePage: _resolvedActivePage } = useEditorDocument({
    pages,
    elements,
    activePageId,
  });
...
const [canvasMode, setCanvasMode] = useState<"page" | "header" | "footer">("page");
  ...
  // Phase 8 View State Extraction Point (Device, Canvas Mode, Preview, Fullscreen Foundation)
  const _editorViewState = useEditorViewState({
    activeDevice,
    setActiveDevice,
    canvasMode,
    setCanvasMode,
...
const navigate = useNavigate();
    ...
    const fetchWebsite = async () => {
      try {
        ...
        if (loadedSite) {
          ...
          const initialHome = (pageId
            ? initialPages.find((p) => p.id === pageId || p.slug === pageId || p.slug === `/${pageId}` || p.name.toLowerCase() === pageId.toLowerCase())
            : undefined) || initialPages.find((p) => p.isHome || p.slug === "/" || p.id === "home") || initialPages[0];
  ...
  const handleInsertGlobalElement = (globalElementId: string) => {
    ...
    _editorHistoryState.dispatchCommand({
      type: "ADD_ELEMENT",
  ...
  const handleDuplicateElement = (idToDuplicate?: string | null, e?: React.MouseEvent) => {
    ...
    const nextElements = _editorHistoryState.dispatchCommand({
      type: "DUPLICATE_ELEMENT",
      elementId: targetId,
    });

    const findDuplicatedId = (tree: EditorElement[]): string | null => {
      for (let i = 0; i < tree.length; i++) {
        if (tree[i].id === targetId && i + 1 < tree.length) {
          return tree[i + 1].id;
        }
        const children = tree[i].children || (tree[i] as any).elements;
        if (Array.isArray(children)) {
          const found = findDuplicatedId(children);
          if (found) return found;
        }
      }
      return null;
    ...
    const newId = findDuplicatedId(nextElements);
    if (newId) {
      setSelectedId(newId);
  ...
  const updateSelectedProp = (key: keyof EditorElement | string, value: any) => {
    if (!selectedId) return;
    if (key !== "navMenuItems" && key !== "megaMenuItems") {
      _editorHistoryState.dispatchCommand({
        type: "UPDATE_PROPS",
    ...
    setElements((prev) =>
      updateTreeElement(prev, selectedId, (el) => {
        ...
        if (key === "navMenuItems" && Array.isArray(value)) {
          updated.content = JSON.stringify(value);
        } else if (key === "megaMenuItems" && Array.isArray(value)) {
          updated.content = JSON.stringify(value);
        }
        return updated;
      })
  ...
  const updateSelectedStyle = (key: keyof ElementStyles, value: any) => {
    if (!selectedId) return;
    if (activeElementState === "normal" && activeDevice === "desktop") {
      _editorHistoryState.dispatchCommand({
        type: "UPDATE_STYLES",
        elementId: selectedId,
        styles: { [key]: value },
    ...
    return (
      ...
      >
        ...
        {el.type === "button" && (() => {
          ...
          const iconColor = el.iconColor || el.buttonColor || mergedStyles.color || el.styles?.color || "#ffffff";
          ...
          return (
            <div style={{ textAlign: mergedStyles.textAlign || "left", width: "100%", boxSizing: "border-box" }}>
              <a
                ...
                style={{
                  backgroundColor: el.buttonBg || mergedStyles.backgroundColor || el.styles?.backgroundColor || "#2563eb",
                  color: el.buttonColor || mergedStyles.color || el.styles?.color || "#ffffff",
                      ...
                      <div className="space-y-3">
                        {(selectedElementAny.navMenuItems || [
                          ...
                          <div
                            ...
                            onDragEnter={(e) => {
                              e.currentTarget.classList.add("border-amber-500", "ring-2", "ring-amber-300");
                            }}
                            onDragLeave={(e) => {
                            ...
                            className="rounded-xl border border-slate-200 bg-white p-3 shadow-xs space-y-2.5 transition hover:border-slate-300"
                          >
                            <div className="flex items-center justify-between gap-1 pb-1 border-b border-slate-100">
                              <span className="text-[11px] font-bold text-slate-800 flex items-center gap-1.5">
                                <span
                                  title="Click and drag to reorder"
                                  className="cursor-grab active:cursor-grabbing text-slate-400 hover:text-amber-600 px-1 py-0.5 text-xs font-bold rounded hover:bg-slate-100 select-none"
                                >
                                  ⋮⋮
                                </span>
                      ...
                      <div className="space-y-3">
                        {(selectedElementAny.megaMenuItems || [
                          ...
                          <div
                            ...
                            onDragEnter={(e) => {
                              e.currentTarget.classList.add("border-blue-500", "ring-2", "ring-blue-300");
                            }}
                            onDragLeave={(e) => {
                              e.currentTarget.classList.remove("border-blue-500", "ring-2", "ring-blue-300");
                            }}
                            onDrop={(e) => {
                              e.preventDefault();
                              e.currentTarget.classList.remove("border-blue-500", "ring-2", "ring-blue-300");
                              ...
                              if (sourceIdxStr) {
                                const sourceIdx = parseInt(sourceIdxStr, 10);
                                if (!isNaN(sourceIdx) && sourceIdx !== cIdx) {
                                  const copy = [...catArr];
                                  const [moved] = copy.splice(sourceIdx, 1);
                                  copy.splice(cIdx, 0, moved);
                          ...
                          >
                            <div className="flex items-center justify-between gap-1 pb-1 border-b border-slate-100">
                              <span className="text-[11px] font-bold text-slate-800 flex items-center gap-1.5">
                                <span
                                  title="Click and drag to reorder category"
                                  className="cursor-grab active:cursor-grabbing text-slate-400 hover:text-blue-600 px-1 py-0.5 text-xs font-bold rounded hover:bg-slate-100 select-none"
                                >
                                  ⋮⋮
                                </span>
                              ...
                              {catItem.columns?.map((col, colIdx) => (
                                <div
                                  ...
                                  onDrop={(e) => {
                                    e.preventDefault();
                                    e.stopPropagation();
                                    const sourceColIdxStr = e.dataTransfer.getData("text/plain");
                                    if (sourceColIdxStr) {
                                      const sourceColIdx = parseInt(sourceColIdxStr, 10);
                                      if (!isNaN(sourceColIdx) && sourceColIdx !== colIdx) {
                                        const cols = [...catItem.columns!];
                                        const [movedCol] = cols.splice(sourceColIdx, 1);
                                        cols.splice(colIdx, 0, movedCol);
                                        const updated = catArr.map((i) =>
                                  ...
                                  className="rounded-xl border border-slate-200 bg-white p-2.5 space-y-2 transition hover:border-slate-300"
                                >
                                  <div className="flex items-center justify-between gap-1 pb-1 border-b border-slate-100">
                                    <div className="flex items-center gap-1.5">
                                      <span
                                        title="Click and drag to reorder column"
                                        className="cursor-grab active:cursor-grabbing text-slate-400 hover:text-blue-600 px-1 py-0.5 text-xs font-bold rounded hover:bg-slate-100 select-none"
                                      >
                                        ⋮⋮
                                      </span>
                                  ...
                                  <div className="space-y-1.5 pt-1">
                                    ...
                                    {col.links.map((link, lIdx) => (
                                      <div
                                        ...
                                        onDragStart={(e) => {
                                          e.stopPropagation();
                                          e.dataTransfer.setData("text/plain", String(lIdx));
                                          e.dataTransfer.effectAllowed = "move";
                                        }}
                                        onDragOver={(e) => {
                                          e.preventDefault();
                                        ...
                                        onDrop={(e) => {
                                          e.preventDefault();
                                          e.stopPropagation();
                                          const sourceLinkIdxStr = e.dataTransfer.getData("text/plain");
                                          if (sourceLinkIdxStr) {
                                            const sourceLinkIdx = parseInt(sourceLinkIdxStr, 10);
                                            if (!isNaN(sourceLinkIdx) && sourceLinkIdx !== lIdx) {
                                              const newLinks = [...col.links];
                                              const [movedLink] = newLinks.splice(sourceLinkIdx, 1);
                                              newLinks.splice(lIdx, 0, movedLink);
                                              const cols = catItem.columns!.map((c, idx) =>
                                        ...
                                        className="rounded border border-slate-100 bg-slate-50 p-2 space-y-1 transition hover:border-slate-200"
                                      >
                                        <div className="flex items-center justify-between gap-1">
                                          <div className="flex items-center gap-1">
                                            <span
                                              title="Click and drag to reorder link"
                                              className="cursor-grab active:cursor-grabbing text-slate-400 hover:text-blue-600 px-1 text-[10px] font-bold rounded hover:bg-slate-200 select-none"
                                            >
                                              ⋮⋮
                                            </span>
