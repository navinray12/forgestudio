...
interface ImportClassesModalProps {
  ...
  initialConflicts: ClassConflictItem[];
  onClose: () => void;
  onConfirmImport: (
...
export const ImportClassesModal: React.FC<ImportClassesModalProps> = ({
  ...
  const [conflicts, setConflicts] = useState<ClassConflictItem[]>(initialConflicts);
  ...
  const handleConfirm = async () => {
    ...
    try {
      const itemsToCreate: CreateClassPayload[] = [];
      const itemsToReplace: { id: string; payload: CreateClassPayload }[] = [];
        ...
        if (c.resolution === "keep_existing") {
          ...
          itemsToReplace.push({
            ...
            payload: {
              ...
              styles: c.importedItem.styles,
              description: c.importedItem.description,
              isGlobal: c.importedItem.isGlobal,
              scope: c.importedItem.scope,
            },
          });
      ...
      setErrorMsg(err?.message || "Failed to import classes.");
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 p-4 backdrop-blur-xs animate-in fade-in duration-150">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="import-classes-modal-title"
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
          <h3 id="import-classes-modal-title" className="text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-slate-100 flex items-center gap-1.5">
            <span>📥</span>
            <span>IMPORT CLASSES PREVIEW</span>
          </h3>
          <button
            type="button"
            onClick={onClose}
        ...
        {newItems.length > 0 && (
          <div className="space-y-2">
            ...
            <div className="max-h-36 overflow-y-auto space-y-1.5 pr-1 border border-slate-200 dark:border-slate-800 rounded-xl p-2 bg-slate-50/50 dark:bg-slate-950/40">
              {newItems.map((item, idx) => (
                <div key={idx} className="flex items-center justify-between text-xs py-1 px-2 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                  <div className="flex items-center gap-1.5">
                    <span className="font-bold">{item.name}</span>
                    <span className="font-mono text-[11px] text-purple-600 dark:text-purple-400">.{item.key}</span>
                  </div>
                  {item.isGlobal ? (
                    <span className="text-[9px] font-extrabold text-purple-700 bg-purple-100 dark:bg-purple-950 px-2 py-0.5 rounded-full border border-purple-300 dark:border-purple-800">
                      🌐 Global
                    </span>
                  ) : (
                    <span className="text-[9px] font-bold text-slate-600 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">
                      🏠 Local
                    </span>
                  )}
                </div>
